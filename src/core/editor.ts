...
import { createDocument, createLayer, cropDocument, editable, EditorError, fail, LIMITS, patchLayer, validateDocument } from './model.ts';
import type { Box, Document, Layer, Stroke, Tool } from './model.ts';
import type { HostConnection, HostRequest } from '../bridge.ts';

export type Operation =
  | {type:'layer.patch';id:string;patch:Partial<Layer>}
  | {type:'layer.remove'|'layer.duplicate';id:string}
  | {type:'layer.reorder';id:string;index:number}
...
export function applyOperation(doc:Document,op:Operation):Document{
  ...
  switch(op.type){
    ...
    case 'layer.remove': editable(doc,op.id);return {...doc,layers:doc.layers.filter(l=>l.id!==op.id)};
    case 'layer.duplicate': {const l=editable(doc,op.id);const index=doc.layers.indexOf(l);return {...doc,layers:[...doc.layers.slice(0,index+1),{...l,id:crypto.randomUUID(),name:`${l.name.slice(0,90)} 副本`},...doc.layers.slice(index+1)]};}
    case 'layer.reorder': {const l=editable(doc,op.id);if(!Number.isInteger(op.index)||op.index<0||op.index>=doc.layers.length)fail('图层位置无效');const layers=doc.layers.filter(x=>x.id!==op.id);layers.splice(op.index,0,l);return {...doc,layers};}
    ...
    case 'stroke':{
      ...
      const l=editable(doc,op.id??'');const key=op.channel==='paint'?'strokes':'mask';return patchLayer(doc,l.id,{[key]:[...l[key],op.stroke]});
    }
...
export class Editor {
  ...
  tool:Tool='move';selected:string|null=null;selection:Box|null=null;
  brush={radius:24,hardness:.75,opacity:1,color:'#ffffff',restore:false};
  preview: {id:string;patch:Partial<Layer>}|null=null;
  ...
  constructor(){this.store.subscribe(()=>{const d=this.store.document;if(this.selected&&!d.layers.some(l=>l.id===this.selected))this.selected=d.layers.at(-1)?.id??null;this.assets.collect(this.store.referencedAssets());this.raster.collect(d);this.queueSave();this.emit();});}
  ...
  async initialize(){
    ...
    try{const checkpoint=await loadRecovery();if(checkpoint){const doc=validateDocument(checkpoint.document),items:Awaited<ReturnType<Assets['prepare']>>[]=[];
      try{for(const id of assetIds(doc)){const a=checkpoint.assets.find(x=>x.id===id);if(!a)fail('恢复素材缺失');const p=await this.assets.prepare(a.blob,id);if(doc.layers.some(l=>l.assetId===id&&(l.width!==p.asset.width||l.height!==p.asset.height))){p.bitmap.close();fail('恢复素材尺寸不一致');}items.push(p);}
  ...
  select(id:string|null){this.selected=id;this.emit();}
  ...
  execute(operations:Operation[],label='编辑',expectedRevision?:number){this.checkIdle();if(!Array.isArray(operations)||!operations.length||operations.length>100)fail('操作数量无效');const ids=new Set(this.doc.layers.map(l=>l.id));this.store.commit(label,d=>operations.reduce(applyOperation,d),expectedRevision);const created=this.doc.layers.find(l=>!ids.has(l.id));if(created){this.selected=created.id;this.emit();}}
  patch(patch:Partial<Layer>){if(!this.selected)return;this.execute([{type:'layer.patch',id:this.selected,patch}],'修改图层');}
  previewPatch(patch:Partial<Layer>){if(!this.active||this.active.locked)return;this.preview={id:this.active.id,patch};this.emit();}
  ...
  async importImages(files:Blob[],names:string[]=[]){
    ...
    try{
      if(this.doc.layers.length+files.length>LIMITS.layers)fail(`最多支持 ${LIMITS.layers} 个图层`);
  ...
  addBlank(){this.execute([{type:'layer.blank'}],'添加空白图层');this.selected=this.doc.layers.at(-1)!.id;this.emit();}
