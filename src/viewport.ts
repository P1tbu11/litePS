...
type Gesture = {kind:'pan';last:Point} | {kind:'box';start:Point} | {kind:'stroke';layer:Layer;stroke:Stroke;channel:'paint'|'mask'};
...
export class Viewport {
  ...
  private down=(e:PointerEvent)=>{
    ...
    if(this.space||e.button===1||tool==='hand')this.gesture={kind:'pan',last:{x:e.clientX,y:e.clientY}};
      ...
      const s:Stroke={points:[point],radius:editor.brush.radius/scale,hardness:editor.brush.hardness,opacity:editor.brush.opacity,color,mode:tool==='erase'?(editor.brush.restore?'restore':'erase'):'paint',...(editor.selection?{clip:corners(editor.selection).map(q=>world?localPoint(q,world):q)}:{})};
